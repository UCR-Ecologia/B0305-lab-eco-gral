...
par(mfrow = c(2, 1))  # 3 plots side by side

for (i in seq_along(thetas)) {
  x <- rnegbin(n, mu = mu, theta = thetas[[i]])
  ...
  # Overlay Poisson(λ = mu) distribution
  xs <- 0:max(x)
  lines(xs, dpois(xs, lambda = mu), col = "red", lwd = 2)
}
...
## Ejemplo: Peligro de Ignorar la Sobredispersión {.smaller}
...
## Modelo Binomial Negativa (CORRECTO) {.smaller}
...
## Ejemplo: Cuándo Usar Modelos Zero-Inflated {.smaller}
...
```{r}
table(data$counts, data$habitat) 
```
...
::: {.callout-tip title="Leer más sobre AIC"}
